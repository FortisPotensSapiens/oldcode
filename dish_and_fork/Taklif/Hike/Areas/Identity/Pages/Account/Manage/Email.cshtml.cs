...
namespace Hike.Areas.Identity.Pages.Account.Manage
{
    public partial class EmailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            if (user == null)
            {
                return NotFound($"Не найден пользватель с ID '{_userManager.GetUserId(User)}'.");
            }
        ...
        public async Task<IActionResult> OnPostChangeEmailAsync()
        {
            ...
            {
                var old = await _userManager.FindByEmailAsync(Input.NewEmail);
                if (old != null)
                {
                    StatusMessage = "Ошибка - пользователь с такой почтой уже зарегестрирован.";
                ...
                StatusMessage = "Писмо для подтверждения выслано вам на почту.";
                return RedirectToPage();
            }

            StatusMessage = "Ваша почта не изменилась.";
            return RedirectToPage();
        }
