...
import { usePostMyPassword } from '~/api/v1/usePostMyPassword';
import { FormField } from '~/components';
import { useDownSm } from '~/hooks';
import { PasswordCreateModel } from '~/types';

type FormProps = { onUpdated: () => void };

const AddPasswordForm: FC<FormProps> = ({ onUpdated }) => {
  const submitForm = usePostMyPassword();
  const { enqueueSnackbar } = useSnackbar();
  const isXs = useDownSm();

  const methods = useForm<PasswordCreateModel>({
    mode: 'all',
  });

  const submitHandler = (event: PasswordCreateModel) => {
    submitForm.mutate(event);
  };

  useEffect(() => {
    if (submitForm.isError) {
      enqueueSnackbar('Ошибка добавления пароля', {
        variant: 'error',
      });
      onUpdated();
    }
  ...
  useEffect(() => {
    if (submitForm.isSuccess) {
      enqueueSnackbar('Пароль успешно добавлен', {
        variant: 'success',
      });
      onUpdated();
    }
