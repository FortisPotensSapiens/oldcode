...
namespace ApeBotApi.Extensions
{
    public static class HttpClientExtensions
    {
        ...
        public static async Task<TResponse> PutAsync<TResponse, TRequest>(this HttpClient client, Uri uri, TRequest request, JsonSerializerOptions options = default, CancellationToken cancellationToken = default)
        {
            var response = await client.PutAsJsonAsync(uri, request, options, cancellationToken);
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            if (!response.IsSuccessStatusCode)
                throw new ApplicationException("Не удалось выполнить запрос")
                {
        ...
        public static async Task<TResponse> DeleteAsync<TResponse>(this HttpClient client, Uri uri, JsonSerializerOptions options = default, CancellationToken cancellationToken = default)
        {
