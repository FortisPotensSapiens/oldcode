...
const TransactionsCashOutTable: React.FC<Props> = ({ transactions, accounts }) => {
    ...
    useEffect(() => {
        ...
        if (accounts[0]?.id)
            axios.get(`${process.env.NEXT_PUBLIC_URL}/api/v1/wallet/account/${accounts[0]?.id}/transactions?dateFrom=${startFrom.toISOString()}&dateTo=${ednTo.toISOString()}`, config)
                .then(res => {
                    if (res.status === 200) {
                        let data = res.data.filter(e => e.accountTransaction.operationType === 'Debiting')
    ...
    return (
        <Box sx={styles.box}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between',
                ...
                <SecondTitle>Заявки</SecondTitle>
                <Box>
            ...
            {rows.length === 0 ? <Typography sx={styles.noData}>Транзакции не найдены</Typography> :
                <Table sx={styles.table} aria-label="caption table">
                    <TableHead sx={{'@media only screen and (max-width: 640px)': {
                            display: "none",
                        }}}>
                        <TableRow>
                            <TableCell align="left">Дата</TableCell>
                            <TableCell align="left">Сумма</TableCell>
                            <TableCell align="left">Комиссия сети</TableCell>
                            <TableCell align="left">Сеть</TableCell>
                        ...
                        {rows.map((row) => (
                            <TableRow key={row.id}  sx={{
                                ...
                                <TableCell align="left">
                                    ...
                                    }}>Комиссия сети</Typography>
                                    {row.debitFee ? row.debitFee + ' USDT' : ' '}
                                </TableCell>
                                <TableCell align="left">
                                    <Typography sx={{
                                        fontSize:'10px',
                                ...
                                <TableCell align="left">
                                    ...
                                    {row.state === 'Created' && <span style={styles.stateCreated}>В процессе</span>}
                                    {row.state === 'Completed' && <span style={styles.stateCompleted}>Завершено</span>}
                                    {row.state === 'Canceled' && <span style={styles.stateCanceled}>Отменено</span>}
                                </TableCell>
                            </TableRow>))}
                    </TableBody>
                </Table>}
...
export default TransactionsCashOutTable;