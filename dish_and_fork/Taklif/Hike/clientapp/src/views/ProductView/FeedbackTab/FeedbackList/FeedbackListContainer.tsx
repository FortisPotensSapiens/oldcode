...
import { useGetProductRatingByFilter } from '~/api/v1/useGetProductRatingByFilter';
...
const FeedbackListContainer = ({ productId }: { productId: string }) => {
  ...
  const { data } = useGetProductRatingByFilter({
    pageNumber: currentPage,
    pageSize: PAGE_SIZE,
    merchId: productId,
  });

  const [allItems, setAllItems] = useState<MerchRatingReadModel[]>([]);
  ...
  return (
    <InfiniteScroll dataLength={allItems.length} next={handleLoad} hasMore={hasMore} loader={<Spin />}>
      <List
        locale={{ emptyText: 'К этому товару пока нет отзывов.' }}
        dataSource={allItems}
        renderItem={(item) => (
